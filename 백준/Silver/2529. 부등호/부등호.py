...
단 아래 예(1)과 같이 첫 자리가 0인 경우도 정수에 포함되어야 한다. 모든 입력에 답은 항상 존재하며 
...
def check(prev_num, curr_num, op):
    """
    prev_num: 바로 앞의 숫자 (문자열 또는 정수)
    curr_num: 지금 넣으려는 숫자 (문자열 또는 정수)
    op: 부등호 기호 ('<' 또는 '>')
    """
    if op == '<':
...
def solve(index, num_str):
    # 현재 만든 문자열(num_str)의 길이가 k+1이 되면
    if index == k + 1:
        # 결과를 리스트에 담고 함수 종료
    ...
    for i in range(10):
        if not visited[i]: # 아직 안 쓴 숫자라면
            # 첫 번째 자리이거나 앞 숫자와 현재 숫자 i가 부등호 조건을 만족하면
            if index == 0 or check(num_str[-1], str(i), diffs[index-1]):
                visited[i] = True      # 사용 처리
                solve(index + 1, num_str + str(i)) # 다음 자리 채우러 감
                visited[i] = False     # 돌아나오면 다시 미사용 처리

# 함수 실행 (0번째 인덱스, 빈 문자열로 시작)
solve(0, "")
# 결과 출력 (오름차순으로 들어가므로 맨 앞과 맨 뒤만 출력)
print(results[-1])
print(results[0])